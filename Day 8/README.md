...
<ul>
  <li><code>increment()</code> increases the current value by <code>1</code> and then returns it.</li>
  <li><code>decrement()</code> reduces the current value by <code>1</code> and then returns it.</li>
  <li><code>reset()</code> sets the current value to <code>init</code> and then returns it.</li>
</ul>

<p>The counter should maintain its state between function calls.</p>
...
<ul>
  ...
  <li><code>calls[i]</code> is one of <code>"increment"</code>, <code>"decrement"</code>, or <code>"reset"</code></li>
</ul>
